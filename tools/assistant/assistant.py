...
@mcp.tool()
async def create_assessment_with_yaml(yaml_content: str, ctx: Context | None = None) -> dict:
    """
    ...
    """
    try:
        logger.info("create_assessment_with_yaml: \n")
        
        if not yaml_content or not yaml_content.strip():
            logger.error("create_assessment_with_yaml error: YAML content is empty\n")
            return {"success": False, "error": "YAML content is empty"}

        try:
            parsed = yaml.safe_load(yaml_content)
            logger.debug("create_assessment_with_yaml yaml_content: {}\n".format(yaml_content))
        except Exception as ye:
            logger.error(f"create_assessment_with_yaml error: Invalid YAML: {ye}\n")
        ...
        if not name or not str(name).strip():
            logger.error("create_assessment_with_yaml error: Assessment name not found in metadata.name\n")
        ...
        if not category_name or not isinstance(category_name, str) or not category_name.strip():
            logger.error("create_assessment_with_yaml error: categoryName not found in metadata.categoryName\n")
        ...
        try:
            ...
            if isinstance(categories_resp, str):
                logger.error(f"create_assessment_with_yaml error: Failed to fetch categories: {categories_resp}\n")
            ...
            if not category_id:
                ...
                if isinstance(create_category_resp, str):
                    logger.error(f"create_assessment_with_yaml error: Failed to create category: {create_category_resp}\n")
                ...
                if isinstance(create_category_resp, dict):
                    ...
                    if not category_id:
                        logger.error(f"create_assessment_with_yaml error: Category created but no ID returned: {create_category_resp}\n")
                        return {"success": False, "error": f"Failed to create category"}
                    
                    logger.info(f"Category '{category_name}' created successfully with ID: {category_id}\n")
                else:
                    logger.error(f"create_assessment_with_yaml error: Unexpected response type when creating category: {type(create_category_resp)}\n")
            ...
            logger.error(f"create_assessment_with_yaml error: Unable to resolve or create category: {e}\n")
        ...
        try:
            ...
            logger.error(f"create_assessment_with_yaml error: Failed to encode YAML content: {be}\n")
        ...
        logger.error("create_assessment_with_yaml error: Unexpected response type: {}\n".format(type(resp)))
...
async def create_assessment(
    ...
    """
    Create a new assessment with category name and assessment name.
    ...
    Args:
        ...
        assessmentName (str): Name of the assessment to create.
...
@mcp.tool() 
async def suggest_asset_controls(    
    controlName: str,
    assessmentId: str = "",
    description: str = "",
    controlId: str = "",
    ...
    """
    Suggest asset controls for a given control name or description.
    
    WORKFLOW / USAGE NOTE:
    This tool should be used ONLY when `suggest_control_config_citations` does not return any suggestions or returns empty results.
    
    This tool searches for matching asset control resources using the provided control name and description as keywords.
    The suggested asset control IDs can then be linked to a target control config using `link_control_config_to_asset_controls`.

    Args:
        controlName (str): Name of the control to get asset suggestions for.
        assessmentId (str, optional): Assessment ID for context.
        description (str, optional): Description of the control to get suggestions for.
        controlId (str, optional): Control ID if updating an existing control config.

    Returns:
        Dict containing:
        - success (bool): Whether the request was successful
        - items (list[dict]): List of matching asset control objects, each containing:
            - id (str): Control ID 
            - name (str): Control name
            - description (str): Control description
            - context (str): Control context
            - assetAssessmentName (str): Asset plan/assessment name (maps from plan_name)
        - next_action (str): Recommended next step ("link_control_config_to_asset_controls")
        - error (str, optional): Error message if request failed
    """
    try:
        logger.info("suggest_asset_controls: \n")

        name_str = str(controlName).strip() if controlName else ""
        desc_str = str(description).strip() if description else ""

        if name_str and desc_str:
            keyword = f"{name_str} - {desc_str}"
        elif name_str:
            keyword = name_str
        else:
            keyword = desc_str
        ...
        if not keyword:
            logger.error("suggest_asset_controls error: controlName or description is required\n")
            return {"success": False, "error": "controlName or description is required"}
        ...
        logger.debug(f"suggest_asset_controls payload: {json.dumps(payload)}\n")

        output = await utils.make_API_call_to_CCow_and_get_response(
            constants.URL_FETCH_MATCHING_ASSET_CONTROLS,
            "POST",
            payload,
            ctx=ctx
        )

        logger.debug("suggest_asset_controls output: {}\n".format(json.dumps(output) if isinstance(output, dict) else output))

        error = utils.handle_error_response(output, "suggest_asset_controls")
        if error:
            logger.error(f"suggest_asset_controls error: {error}\n")
            return error
            ...
            if isinstance(raw_items, list):
                for raw_item in raw_items:
                    if isinstance(raw_item, dict):
                        ...
                        abstracted_item = {
                            "id": raw_item.get("resourceId") or payload.get("plan_control_id") or raw_item.get("id", ""),
                            "name": payload.get("name") or raw_item.get("name", ""),
                            "description": payload.get("description") or raw_item.get("description", ""),
                            "context": payload.get("context") or raw_item.get("context", ""),
                            "assetAssessmentName": payload.get("plan_name") or raw_item.get("plan_name", "")
                        }
                        items.append(abstracted_item)

        logger.info(f"suggest_asset_controls: Found {len(items)} matching item(s)\n")
        return {"success": True, "items": items, "next_action": "link_control_config_to_asset_controls"}
...
@mcp.tool() 
async def link_control_config_to_asset_controls(
    sourceAssetControlIds: list[str],
    targetControlConfigId: str,
    ctx: Context | None = None
) -> dict: 
    """
    Link one or more source asset controls to a target control config.

    This tool establishes linking between source asset controls and a target control config in an assessment.

    Args:
        sourceAssetControlIds (list[str]): List of source asset control IDs to link.
        targetControlConfigId (str): Target control config ID to link the source asset controls to.

    Returns:
        Dict containing:
        - success (bool): Whether the linking operation was successful
        - message (str, optional): Success description
        - next_action (str): Recommended next step ("fetch_control_source_summary")
        - data (dict, optional): Raw response from the link API
        - error (str, optional): Error message if request failed
    """
    try:
        logger.info("link_control_config_to_asset_controls: \n")

        if not sourceAssetControlIds or not isinstance(sourceAssetControlIds, list) or len(sourceAssetControlIds) == 0:
            logger.error("link_control_config_to_asset_controls error: sourceAssetControlIds is required and must be a non-empty list\n")
            return {"success": False, "error": "sourceAssetControlIds is required and must be a non-empty list"}

        if not targetControlConfigId or not str(targetControlConfigId).strip():
            logger.error("link_control_config_to_asset_controls error: targetControlConfigId is required\n")
            return {"success": False, "error": "targetControlConfigId is required"}

        target_id = str(targetControlConfigId).strip()
        valid_source_ids = [str(sid).strip() for sid in sourceAssetControlIds if sid and str(sid).strip()]

        if not valid_source_ids:
            logger.error("link_control_config_to_asset_controls error: sourceAssetControlIds contains no valid strings\n")
            return {"success": False, "error": "sourceAssetControlIds contains no valid control IDs"}
        ...
        logger.debug(f"link_control_config_to_asset_controls payload: {json.dumps(payload)}\n")
        ...
        error = utils.handle_error_response(output, "link_control_config_to_asset_controls")
        if error:
            logger.error(f"link_control_config_to_asset_controls error: {error}\n")
            return error

        logger.info(f"link_control_config_to_asset_controls: Successfully linked {len(valid_source_ids)} source asset control(s) to target {target_id}\n")
        return {"success": True, "message": "Source asset controls were successfully linked to the target control config.", "next_action": "fetch_control_source_summary", "data": output}

    except Exception as e:
        logger.error(traceback.format_exc())
        logger.error("link_control_config_to_asset_controls error: {}\n".format(e))
        return {"success": False, "error": f"Unexpected error linking asset controls to target control config: {e}"}
...
async def create_control_config(
    ...
    """
    Create or update an assessment by adding a control config.

    Important:
    - Use only user-provided inputs and do not assume any values.

    Behavior:
    - If `entityClass` and `entities` are provided, uses the entity-based control creation flow.
    - If `entityClass` or `entities` are missing/empty, uses direct control config creation flow.
    ...
    Returns:
        Dict with success status and created control/assessment details or error message.
    """
    try:
        ...
        if not controlDescription or not str(controlDescription).strip():
            return {"success": False, "error": "controlDescription is mandatory"}
        
        if not controlCategory or not str(controlCategory).strip():
            return {"success": False, "error": "controlCategory is mandatory"}

        # Flow A: If entityClass and entities are both provided, use existing add-control-objective API
        if entityClass and str(entityClass).strip() and isinstance(entities, list) and len(entities) > 0:
            valid_entities = [e.strip() for e in entities if isinstance(e, str) and e.strip()]
            if valid_entities:
                entity_payload = [
                    {
                        "name": entity,
                ...
                payload = {
                    ...
                    "controlObjectives": [
                        {
                            "name": controlName.strip(),
                            "description": controlDescription.strip(),
                            "category": controlCategory.strip(),
                ...
                logger.debug("create_control_config payload (entity flow):\n{}\n".format(json.dumps(payload, indent=2)))
                ...
                logger.debug(
                    "create_control_config response (entity flow):\n{}\n".format(
                        json.dumps(resp) if isinstance(resp, dict) else resp
                    )
                )
                ...
                if isinstance(resp, dict):
                    ...
                    logger.info(
                        f"create_control_config: Successfully created custom control with ID: {result.get('assessment_id')}\n"
                    )
        ...
        # Flow B: Direct control config creation without entities
        logger.info("create_control_config: executing direct control creation flow (no entities provided)\n")
        
        # Step 1: Resolve assessmentId (planId)
        assessment_id = None
        category_name = str(controlCategory).strip()
        assessment_name = str(assessmentName).strip()

        try:
            plans_resp = await utils.make_GET_API_call_to_CCow(
                f"{constants.URL_PLANS}?fields=basic&name_contains={assessment_name}",
                ctx=ctx
            )
            if isinstance(plans_resp, dict) and "items" in plans_resp:
                items = plans_resp.get("items", [])
                for item in items:
                    if isinstance(item, dict) and item.get("name") == assessment_name:
                        assessment_id = item.get("id")
                        break
                if not assessment_id and items and isinstance(items[0], dict):
                    assessment_id = items[0].get("id")
        except Exception as pe:
            logger.error(f"create_control_config: Error fetching plans: {pe}\n")

        # If assessment plan does not exist, return error directly
        if not assessment_id:
            logger.error(f"create_control_config: Assessment '{assessment_name}' not found\n")
            return {"success": False, "error": f"Assessment '{assessment_name}' not found"}

        # Step 2: Fetch existing controls for this plan to find/increment parents and children
        existing_controls = []
        try:
            ctrl_resp = await utils.make_GET_API_call_to_CCow(
                f"{constants.URL_PLAN_CONTROLS}?page=1&page_size=1000&plan_id={assessment_id}&fields=basic&include_additional_context=true",
                ctx=ctx
            )
            if isinstance(ctrl_resp, dict) and "items" in ctrl_resp and isinstance(ctrl_resp["items"], list):
                existing_controls = ctrl_resp["items"]
        ...
        child_payload = {
            "name": str(controlName).strip(),
            "description": str(controlDescription).strip() if controlDescription else "",
        ...
        logger.debug(f"create_control_config child payload: {json.dumps(child_payload)}\n")
        child_resp = await utils.make_API_call_to_CCow_and_get_response(
            constants.URL_PLAN_CONTROLS,
            "POST",
            child_payload,
            ctx=ctx
        ...
        if isinstance(child_resp, str):
            return {"success": False, "error": child_resp}
        if isinstance(child_resp, dict) and "Message" in child_resp:
            return {"success": False, "error": child_resp}

        if isinstance(child_resp, dict) and child_resp.get("id"):
            control_data = {
                "id": child_resp.get("id"),
            ...
            return {"success": True, "data": control_data}

        return {"success": False, "error": f"Failed to create child control: {child_resp}"}

    except Exception as e:
        logger.error(traceback.format_exc())
        logger.error(f"create_control_config error: {e}\n")
        return {
            "success": False,
            "error": f"Unexpected error while adding control config: {e}"
        }
...
async def update_control_config_contexts(
    ...
    """
    Update both context and additionalContext (entities) of an existing control config.
...
async def assitant_list_all_assessment_categories(ctx: Context | None = None) -> assessment_vo.CategoryListV2VO:
    """
    ...
    try:
        ...
        if error:
            ...
            return assessment_vo.CategoryListV2VO(error=error)

        category_list: list[assessment_vo.CategoryVO] = []
        for item in output:
            if "id" in item and "name" in item:
                category_list.append(assessment_vo.CategoryVO(id=item["id"],name=item["name"]))
        
        logger.debug("categories: {}\n".format(category_list))
        return assessment_vo.CategoryListV2VO(categories=category_list)
    except Exception as e:
        logger.error("list_all_assessment_categories error: {}\n".format(e))
